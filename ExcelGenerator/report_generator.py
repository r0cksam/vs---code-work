...
report_generator.py
═══════════════════════════════════════════════════════════════════════
Single entry-point for all lake reports.

  1 · Matrix Report       – full rebuild every run (xlsxwriter, constant_memory)
  2 · Detailed Analysis   – append-aware flat table; only new dates are queried
  3 · Both reports

Run:
    python report_generator.py
    python report_generator.py <lake_folder>   # skips the lake-path prompt

Requires:
    duckdb, pandas, pyarrow, xlsxwriter, psutil, openpyxl (for pd.read_excel)
"""
...
# ═══════════════════════════════════════════════════════════════
# SECTION A — SHARED PROMPTS / UTILITIES
# ═══════════════════════════════════════════════════════════════

def prompt_path(question: str, must_be_dir: bool = True) -> Path:
    """Loop until the user gives an existing path of the right type."""
    while True:
...
def prompt_optional(question: str, default: str = "") -> str | None:
    """Ask for optional input; return default (or None) when user presses Enter."""
...
def prompt_year_month(lake_root: Path) -> tuple[str | None, str | None]:
    """Ask the user for an optional year / month filter, with lake-aware hints."""
    ...
    try:
        ...
        while True:
            ...
            if not y_in.isdigit() or not (2000 <= int(y_in) <= 2100):
                print("  ❌ Invalid year")
                continue
            if available_years and y_in not in available_years:
                print(f"  ⚠️  year={y_in} not found. Continue? (y/n): ", end="")
                if input().strip().lower() != "y":
                    continue
            break
        ...
        log.info(f"Filter: year={year_str}, month={month_str}")
...
# ═══════════════════════════════════════════════════════════════
# SECTION B — MATRIX REPORT
# ═══════════════════════════════════════════════════════════════

# ── B.1  Config dataclass ───────────────────────────────────────
...
# ── B.2  Auto-scale based on available RAM / CPU ────────────────
...
def auto_config(lake_root: Path) -> MatrixConfig:
    ...
    top_n = min(top_n, 1_048_576 - 5)   # Excel row limit
...
# ── B.3  PyArrow helpers (schema + row-count via metadata) ──────
...
def pa_schema(
    ...
    for pf in scan_root.rglob("*.parquet"):
        try:
            s = pq.read_schema(pf)
            for name in s.names:
                if name not in skip and name not in schema_map:
                    schema_map[name] = str(s.field(name).type)
        except Exception as exc:
            log.warning(f"Schema read skipped {pf.name}: {exc}")
        if len(schema_map) > 5:
            break
    return schema_map
...
def _discover_day_partitions(
    ...
    def _collect(scan_root: Path, y: str, mo: str) -> None:
        for day_dir in sorted(scan_root.iterdir(), reverse=True):
    ...
    if year and month:
        mo_dir = lake_root / f"year={year}" / f"month={month}"
        if mo_dir.exists():
            _collect(mo_dir, year, month)
    elif year:
        y_dir = lake_root / f"year={year}"
        if y_dir.exists():
            for mo_dir in sorted(y_dir.iterdir(), reverse=True):
                if not mo_dir.is_dir() or not mo_dir.name.startswith("month="):
                    continue
                mo = mo_dir.name[6:]
                _collect(mo_dir, year, mo)
                if len(days) >= max_days:
                    break
    else:
        for y_dir in sorted(lake_root.iterdir(), reverse=True):
            if not y_dir.is_dir() or not y_dir.name.startswith("year="):
                continue
            y = y_dir.name[5:]
            for mo_dir in sorted(y_dir.iterdir(), reverse=True):
                if not mo_dir.is_dir() or not mo_dir.name.startswith("month="):
...
def build_overview(
    ...
    _kv(ws, row, "Lake folder",   str(cfg.lake_root),  s); row += 1
    ...
    total_files   = pa_total_file_count(
        cfg.lake_root / f"year={cfg.year}" if cfg.year else cfg.lake_root
    )
...
def build_ip_matrix_with_state_channel(
    ...
    log.info("Building Client IP matrix (with State & Channel) …")
...
def build_top_channels_by_state(
    ...
    for i, r in enumerate(df.iterrows(), 1):
        ...
        is_alt = i % 2 == 0
        d, n   = (s.alt, s.num_alt) if is_alt else (s.data, s.num)
        ws.write(i, 0, row_data["state"],           d)
        ws.write(i, 1, row_data["channel"],         d)
        ws.write(i, 2, int(row_data["requests"]),   n)
        ws.write(i, 3, int(row_data["rank"]),       n)
...
def build_device_matrix_with_state(
    ...
    log.info("Building Device matrix (with State) …")
    ...
    matrix_data = _matrix_counts_by_day(con, day_parts, device_col_expr, top_devs, "device")
    if not matrix_data:
...
# ── B.9  Matrix report entry point ──────────────────────────────

def run_matrix_report(lake_root: Path) -> None:
    """Interactive prompts → build the full matrix Excel report."""
    ...
    cfg.year, cfg.month = prompt_year_month(lake_root)
    ...
    log.info("Detecting schema via PyArrow …")
...
# ═══════════════════════════════════════════════════════════════
# SECTION C — DETAILED ANALYSIS  (append-aware)
# ═══════════════════════════════════════════════════════════════

def run_detailed_analysis(lake_root: Path) -> None:
    """
    Generate (or update) the flat detailed analysis Excel file.

    First run  → creates  <output_dir>/detailed_analysis.xlsx
    Later runs → queries only new dates, merges with existing data,
                 rewrites the file so formatting stays consistent.
    """
    print("\n" + "═" * 58)
    print("  Detailed Analysis  –  append-aware")
    print("═" * 58 + "\n")
    ...
    if not is_first_run:
        existing_dates = get_existing_dates(output_file, DETAILED_SHEET_NAME)
        print(f"\n  📋  Found existing file with {len(existing_dates)} date(s) already loaded.")
        if existing_dates:
    ...
    print(f"  Mode        : {'CREATE (first run)' if is_first_run else 'APPEND (new dates only)'}")
    print("─" * 58)
    ...
    sql = f"""
        ...
        ORDER BY utc_date, state, {ua_col}, channel, platform, device
    """
    ...
    # ── Filter out dates already in the file ──────────────────────
    if existing_dates:
        before = len(df)
        df = df[~df["Date (IST)"].isin(existing_dates)]
        skipped = before - len(df)
        if skipped:
            print(f"  ℹ️   Skipped {skipped:,} rows for {len(existing_dates)} already-loaded date(s).")

    if df.empty:
        print("  ✅  Nothing new to add — your file is already up to date.")
        return

    new_date_count = df["Date (IST)"].nunique()
    print(f"  ➕  {new_date_count} new date(s) to add ({len(df):,} rows).")

    # ── Compute % of requests per day (across all data in this batch) ─
    ...
    if is_first_run:
        ...
        print("  🔄  Merging with existing data and rewriting file …")
        merge_and_rewrite(
            output_path  = output_file,
            new_df       = output_df,
            sheet_name   = DETAILED_SHEET_NAME,
            date_column  = "Date (IST)",
        )
        print(f"\n  ✅  Updated: {output_file}")

    print("  💡  Open in Excel → Insert Pivot Table → slicers on State, Channel, Platform, Device.")
...
def main() -> None:
    print("\n" + "═" * 58)
    print("  Lake Report Generator")
    print("═" * 58)

    # ── Resolve lake folder (CLI arg or interactive) ─────────────
    if len(sys.argv) > 1:
        ...
        lake_root = prompt_path("  Lake folder path: ", must_be_dir=True)
    ...
    print("    1 · Matrix Report       (full rebuild, all dimension matrices)")
    print("    2 · Detailed Analysis   (flat table, append-aware – skips loaded dates)")
    ...
    if choice in ("1", "3"):
        run_matrix_report(lake_root)

    if choice in ("2", "3"):
        run_detailed_analysis(lake_root)
