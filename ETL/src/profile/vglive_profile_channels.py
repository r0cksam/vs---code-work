...
DEFAULT_LAKE = Path(os.getenv("VG_DASH_LAKE_ROOT", os.getenv("VG_ETL_LAKE_ROOT", str(ETL_ROOT / "data" / "lake"))))
DEFAULT_OUT = Path(os.getenv("VG_DASH_PROFILE_DIR", ETL_ROOT / "output" / "watch_hours" / "channel_review"))
...
SELECT
    ...
    MIN({ist_timestamp_expr("req_epoch")}) AS first_seen,
    MAX({ist_timestamp_expr("req_epoch")}) AS last_seen,
    ...
    con.execute(f"""
        COPY (
            SELECT
                ...
                MIN({ist_timestamp_expr("reqTimeSec")}) FILTER (WHERE statusCode = '200' AND reqPath LIKE '%.ts') AS first_ts_time,
                MAX({ist_timestamp_expr("reqTimeSec")}) FILTER (WHERE statusCode = '200' AND reqPath LIKE '%.ts') AS last_ts_time
            FROM read_parquet('{parquet_glob}', hive_partitioning=1)
