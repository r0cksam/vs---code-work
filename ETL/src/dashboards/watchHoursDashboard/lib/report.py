...
DEFAULT_UA_LOOKUP_PATH = Path(
    os.getenv(
        "VG_UA_DECODE_LOOKUP",
        str(Path(__file__).resolve().parents[4] / "output" / "device_decode" / "ua_decode_lookup_both_all.parquet"),
    )
)
...
def _overlay_final_ua_lookup(usage: pd.DataFrame, lookup_path: Path = DEFAULT_UA_LOOKUP_PATH) -> pd.DataFrame:
    if usage.empty or "ua_hash" not in usage.columns or not lookup_path.exists():
        return usage
    lookup = _read_parquet_safe(lookup_path)
    ...
    lookup = lookup[[c for c in ["ua_hash", *decode_cols] if c in lookup.columns]].drop_duplicates("ua_hash", keep="last")
    merged = usage.merge(lookup, on="ua_hash", how="left", suffixes=("", "_final"))
    for col in decode_cols:
        final_col = f"{col}_final"
        if final_col not in merged.columns:
            continue
        if col not in merged.columns:
            merged[col] = merged[final_col]
        else:
            has_final = merged[final_col].fillna("").astype(str).str.strip().ne("")
            merged.loc[has_final, col] = merged.loc[has_final, final_col]
        merged = merged.drop(columns=[final_col])
    merged["final_lookup_applied"] = True
    return merged
...
def load_device_decode_outputs(folder: Path = DEFAULT_DEVICE_DECODE_DIR) -> dict:
    ...
    ua_cache = _overlay_final_ua_lookup(ua_cache)
    ...
    return {
        ...
        "files": {
            ...
            "ua_lookup": str(DEFAULT_UA_LOOKUP_PATH) if DEFAULT_UA_LOOKUP_PATH.exists() else "",
        ...
        "ua_cache": records(
            ...
            [
                ...
                "form_factor", "model_code", "product_family", "generation", "app_player",
                "api_device_type", "api_brand", "api_model", "api_os_name", "api_browser_name",
                "confidence", "final_lookup_applied",
            ],
        ),
    }
