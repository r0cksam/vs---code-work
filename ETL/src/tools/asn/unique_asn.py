...
1) From parquet folder/glob:
   python ./src/tools/asn/unique_asn.py ^
     --input "./cleaned_output" ^
     --asn-col asn ^
     --output "./data/asn/asn_summary_with_names.csv"

2) From an existing CSV with columns like value,count,% of rows:
   python ./src/tools/asn/unique_asn.py ^
     --csv "./data/asn/unique_asn.csv" ^
     --output "./data/asn/asn_summary_with_names.csv"
...
ETL_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_ASN_DIR = ETL_ROOT / "data" / "asn"


def parse_args():
    p = argparse.ArgumentParser(description="Build ASN summary with ASN names.")
    src = p.add_mutually_exclusive_group(required=True)
    src.add_argument("--input", help="Parquet folder or parquet glob, e.g. ./data or ./data/*.parquet")
    ...
    p.add_argument("--output", default=str(DEFAULT_ASN_DIR / "asn_summary_with_names.csv"), help="Output CSV path")
    p.add_argument("--cache", default=str(DEFAULT_ASN_DIR / "asn_lookup_cache.json"), help="ASN lookup cache JSON path")
