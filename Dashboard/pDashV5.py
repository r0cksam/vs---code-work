...
with tab5:
    ...
    with ub_scan_col2:
        scan_devices_btn = st.button("🔍 Scan device IDs", type="primary", key="ub_scan_btn")
    ...
    if load_all_btn or st.session_state.get("ub_loaded_device") == selected_device:
        _bar2 = st.progress(0, text="Querying parquet files ...")
        _ph2  = st.empty()
        _stages2 = [
            (5,  "Opening parquet files"),
    ...
    if run_dash_btn:
        ...
        _raw_df = ub_staged_progress(_rbar, _rph, _stages_run,
                                      lambda: ub_load_device(pq_glob, selected_device,
                                                             str(start_date), str(end_date), cm))
