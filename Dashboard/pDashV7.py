...
PLATFORM_SUFFIX_PATTERN = r"_(FireTV|AndroidTV|Android|firetv|androidtv|android|Web|web)$"

def normalize_channel_name(x: str) -> str:
    if pd.isna(x):
        return ""
    s = str(x).strip()
    s = re.sub(r"\s*_\s*", "_", s)
    s = re.sub(PLATFORM_SUFFIX_PATTERN, "", s, flags=re.IGNORECASE)
    s = re.sub(r"\s+", " ", s).strip()
    return s

def extract_channel_platform_suffix(x: str) -> str:
    if pd.isna(x):
        return ""
    s = str(x).strip()
    s = re.sub(r"\s*_\s*", "_", s)
    m = re.search(PLATFORM_SUFFIX_PATTERN, s, flags=re.IGNORECASE)
...
def ub_enrich(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["channel_name"] = out["channel_name_raw"].map(normalize_channel_name).replace("", "Unknown")
...
@st.cache_data(show_spinner="Loading clean channel master ...", ttl=900)
def gb_get_channel_master(parquet_glob: list, col_map: dict, start_date: str, end_date: str, limit_rows: int = 500) -> pd.DataFrame:
    con = get_db_conn()
    cte = gb_behavior_cte(parquet_glob, col_map, start_date, end_date, "Default")
    query = cte + f"""
    SELECT
        channel_name AS clean_channel,
    ...
    enriched AS (
        SELECT
            *,
            ...
            regexp_replace(
                regexp_replace(
                    COALESCE(NULLIF(channel_qs, ''), NULLIF(regexp_extract(reqPath, '(vglive-sk-\d+)', 1), ''), 'Unknown'),
                    '\s*_\s*',
                    '_'
                ),
                '_(FireTV|AndroidTV|Android|firetv|androidtv|android|Web|web)$',
                ''
            ) AS channel_name,
...
with tab6:
    ...
    gb_show_raw_debug = False
    if gb_entity == "Channel":
        st.info("Channel analytics use **Clean Channel** by default. Platform stays separate in the `platform` field, so values like `IndiaTV_FireTV` and `IndiaTV_AndroidTV` are grouped under `IndiaTV`.")
        gb_show_raw_debug = st.checkbox("Show raw channel debug mode", value=False, key="gb_show_raw_debug")
        if gb_show_raw_debug:
            gb_channel_mode = st.radio("Channel mode", ["Clean", "Raw"], horizontal=True, key="gb_channel_mode")
        st.caption("Use **Clean** for business reporting. Use **Raw** only to debug incoming query-string values.")
    ...
    if gb_entity == "Channel" and gb_channel_mode == "Clean":
        with st.expander("📤 Pure Channel Master (clean channel + separate platform)", expanded=False):
            with st.spinner("Building clean channel master ..."):
                channel_master_df = gb_get_channel_master(gb_glob, gb_cm, gb_start_s, gb_end_s, limit_rows=1000)
            if channel_master_df.empty:
                st.info("No clean channel rows found for this date range.")
            else:
                st.caption("`clean_channel` is the business channel name. `platform` stays separate, so platform suffixes are removed from the channel value.")
                st.dataframe(channel_master_df, use_container_width=True, hide_index=True, height=320)
                st.download_button(
                    "📥 Download Pure Channel Master CSV",
                    data=channel_master_df.to_csv(index=False).encode("utf-8"),
                    file_name=f"pure_channel_master_{gb_start_s}_to_{gb_end_s}.csv",
                    mime="text/csv",
                    key="gb_dl_channel_master",
                )
