...
def column_completeness_profile(folder_key: str, profile_cols: list, batch_size: int = 50000) -> pd.DataFrame:
    """
    Scan selected parquet folders and calculate filled/empty percentages per column.
    Empty/null-like means: real NULL, blank string, '-' or '^'.
    If a column is missing from a parquet file, every row in that file is counted as empty for that column.
    """
    files = collect_files(folder_key.split("|"))
    file_cols = get_file_columns(folder_key)
    profile_cols = list(dict.fromkeys([c for c in profile_cols if c]))

    stats = {c: {"total_rows": 0, "empty_rows": 0} for c in profile_cols}
    total_files = len(files)

    bar = st.progress(0, text=f"Profiling column completeness ... 0 / {total_files:,} files")

    for i, f in enumerate(files):
        pct = int((i + 1) / total_files * 100) if total_files else 100
        bar.progress(pct, text=f"Profiling column completeness ... {i+1:,} / {total_files:,} files")

        try:
            meta = pq.read_metadata(f)
            n_rows_file = int(meta.num_rows or 0)
        except Exception:
            n_rows_file = 0

        available = file_cols.get(str(f), set())
        present_cols = [c for c in profile_cols if c in available]
        missing_cols = [c for c in profile_cols if c not in available]

        # Missing column in a file = empty for every row in that file.
        for c in missing_cols:
            stats[c]["total_rows"] += n_rows_file
            stats[c]["empty_rows"] += n_rows_file

        for c in present_cols:
            stats[c]["total_rows"] += n_rows_file

        if not present_cols or n_rows_file == 0:
            continue

        try:
            pf = pq.ParquetFile(f)
            for batch in pf.iter_batches(columns=present_cols, batch_size=batch_size):
                ...
                for c in present_cols:
                    try:
                        ...
                        # Convert to string to catch blank, '-' and '^' consistently.
                        # Nulls stay null after cast, so we combine this with the null mask.
                        s_arr = pc.cast(arr, pa.string(), safe=False)
                        s_arr = pc.utf8_trim_whitespace(s_arr)

                        blank_mask = pc.equal(s_arr, pa.scalar("", pa.string()))
                        dash_mask  = pc.equal(s_arr, pa.scalar("-", pa.string()))
                        caret_mask = pc.equal(s_arr, pa.scalar("^", pa.string()))

                        empty_mask = pc.or_kleene(
                            null_mask,
                            pc.or_kleene(blank_mask, pc.or_kleene(dash_mask, caret_mask))
                        )
                        empty_count = pc.sum(pc.cast(pc.fill_null(empty_mask, False), pa.int64())).as_py()
                        stats[c]["empty_rows"] += int(empty_count or 0)
                    except Exception:
                        # If a strange column cannot be cast/profiled, count only true nulls as empty.
                        try:
                            null_count = pc.sum(pc.cast(pc.is_null(tbl.column(c)), pa.int64())).as_py()
                            stats[c]["empty_rows"] += int(null_count or 0)
                        except Exception:
                            pass
        except Exception:
            # If file read fails, mark present cols as unknown-empty = 0 extra empty beyond already counted total.
            continue
    ...
    for c, s in stats.items():
        total = int(s["total_rows"] or 0)
        empty = int(s["empty_rows"] or 0)
...
with tab1:
    ...
    with prof_exp:
        st.caption("Counts real NULL, blank values, `-`, and `^` as empty/null-like. Missing columns in a file are also counted as empty for that file's rows.")
        ...
        if profile_mode == "Selected columns only":
        ...
        if run_profile:
            if not profile_cols:
                ...
                prof_df = column_completeness_profile(folder_key, list(profile_cols))
                st.session_state["col_profile_df"] = prof_df
                st.session_state["col_profile_key"] = folder_key
                st.success(f"Profile complete for {len(prof_df):,} column(s).")
