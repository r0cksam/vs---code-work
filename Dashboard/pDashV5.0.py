...
def ub_get_qsa_device_ids() -> list:
    qsa_df = st.session_state.get("qsa_parsed_df")
    if qsa_df is None or getattr(qsa_df, "empty", True) or "device_id" not in qsa_df.columns:
        return []
    vals = (
        qsa_df["device_id"]
        .dropna()
...
def ub_get_device_date_bounds(parquet_glob: str, device_id: str, col_map: dict):
    con = get_db_conn()
...
@st.cache_data(show_spinner="Loading all device data ...", ttl=900)
def ub_load_device_all(parquet_glob: str, device_id: str, col_map: dict) -> pd.DataFrame:
    con  = get_db_conn()
    qs   = _cm(col_map, "queryStr")
    ts   = _cm(col_map, "reqTimeSec")
...
def ub_build_pdf_report(window_df: pd.DataFrame, sess_df: pd.DataFrame, content_window: pd.DataFrame,
                        selected_device: str, start_date: str, end_date: str,
                        n_sessions: int, est_watch_hrs: float, top_channel: str,
                        top_content: str, top_dev_type: str) -> bytes:
    ...
    out = io.BytesIO()
    doc = SimpleDocTemplate(out, pagesize=A4, rightMargin=24, leftMargin=24, topMargin=24, bottomMargin=24)
    story = []

    story.append(Paragraph("User Behavior Dashboard Report", styles["Title"]))
    story.append(Paragraph(f"Device ID: {selected_device}", styles["Normal"]))
    story.append(Paragraph(f"Date range: {start_date} to {end_date}", styles["Normal"]))
    story.append(Spacer(1, 8))

    summary_data = [
        ["Rows", f"{len(window_df):,}", "Sessions", f"{n_sessions:,}"],
        ["Est. Watch Hrs", str(est_watch_hrs), "Top Channel", str(top_channel)],
        ["Top Content", str(top_content), "Device Type", str(top_dev_type)],
    ]
    tbl = Table(summary_data, colWidths=[1.3*inch, 1.6*inch, 1.3*inch, 2.8*inch])
    ...
    if not window_df.empty:
        ...
        ax.plot(min_act["event_time"], min_act["requests"])
        ax.set_title("Minute-by-minute activity")
        ax.set_xlabel("Event time")
        ax.set_ylabel("Requests")
        ax.grid(True, alpha=0.25)
        fig.autofmt_xdate()
        story.append(Paragraph("Activity chart", styles["Heading2"]))
        story.append(Image(_fig_to_buf(fig), width=7.3*inch, height=2.5*inch))
        story.append(Spacer(1, 10))

        channel_counts = window_df["channel_name"].value_counts().head(10)
        if not channel_counts.empty:
            fig, ax = plt.subplots(figsize=(9, 3.6))
            ax.barh(channel_counts.index.astype(str)[::-1], channel_counts.values[::-1])
            ax.set_title("Top channels by requests")
            ax.set_xlabel("Requests")
            story.append(Paragraph("Top channels", styles["Heading2"]))
            story.append(Image(_fig_to_buf(fig), width=7.3*inch, height=2.8*inch))
            story.append(Spacer(1, 10))

    top_content_df = content_window.head(12).copy()
    if not top_content_df.empty:
        fig, ax = plt.subplots(figsize=(9, 4.2))
        labels = top_content_df["content_label"].astype(str).str.slice(0, 45)
        ax.barh(labels[::-1], top_content_df["est_watch_min"].values[::-1])
        ax.set_title("Top content by estimated watch minutes")
        ax.set_xlabel("Estimated watch minutes")
        story.append(Paragraph("Top content", styles["Heading2"]))
        story.append(Image(_fig_to_buf(fig), width=7.3*inch, height=3.2*inch))
        story.append(Spacer(1, 10))

    table_df = content_window[[c for c in ["content_label", "channel_name", "requests", "est_watch_min", "sessions"] if c in content_window.columns]].head(20).copy()
    if not table_df.empty:
        story.append(PageBreak())
        story.append(Paragraph("Content summary", styles["Heading2"]))
        table_rows = [list(table_df.columns)] + table_df.astype(str).values.tolist()
    ...
    if not sess_df.empty:
        sess_small = sess_df[[c for c in ["session_key", "start", "end", "requests", "est_watch_min", "top_channel"] if c in sess_df.columns]].head(20).copy()
        if not sess_small.empty:
            story.append(Spacer(1, 12))
            story.append(Paragraph("Session summary", styles["Heading2"]))
            sess_rows = [list(sess_small.columns)] + sess_small.astype(str).values.tolist()
            tbl = Table(sess_rows, repeatRows=1)
            tbl.setStyle(TableStyle([
                ("BACKGROUND", (0,0), (-1,0), colors.lightgrey),
                ("BOX", (0,0), (-1,-1), 0.5, colors.black),
                ("INNERGRID", (0,0), (-1,-1), 0.25, colors.grey),
...
with tab5:
    ...
    # ── Device ID scan / reuse ───────────────────────────────
    st.markdown("#### Step 1 — Select a device")

    qsa_device_ids = ub_get_qsa_device_ids()
    ub_scan_col1, ub_scan_col2, ub_scan_col3 = st.columns([2, 1, 1])
    with ub_scan_col2:
        reuse_qsa_btn = st.button("⚡ Reuse IDs from QSA", key="ub_reuse_qsa_btn", disabled=not qsa_device_ids)
    with ub_scan_col3:
        scan_devices_btn = st.button("🔍 Scan parquet IDs", type="primary", key="ub_scan_btn")

    if reuse_qsa_btn and qsa_device_ids:
    ...
    with ub_scan_col1:
        if qsa_device_ids:
            st.caption(f"Query String Analyzer already found {len(qsa_device_ids):,} device IDs — reuse them instantly.")
        elif st.session_state.ub_device_ids:
            st.caption(f"{len(st.session_state.ub_device_ids):,} device IDs found")

    if not st.session_state.ub_device_ids and qsa_device_ids:
        st.session_state.ub_device_ids = qsa_device_ids
    ...
    if manual_device.strip():
        ...
        st.info("Click 'Scan device IDs' above or type a device_id manually to continue.")
        st.stop()

    # ── Date range (fast metadata query) ─────────────────────
    st.markdown("#### Step 2 — Pick date range")

    load_all_btn = st.button("📅 Load date range for this device", key="ub_load_dates", type="secondary")

    if selected_device != st.session_state.get("ub_loaded_device"):
        st.session_state["ub_date_bounds"] = None
        st.session_state["ub_window_df"] = None
        st.session_state["ub_sess_df"] = pd.DataFrame()

    if load_all_btn or st.session_state.get("ub_loaded_device") == selected_device:
        min_dt, max_dt = ub_get_device_date_bounds(pq_glob, selected_device, cm)
        if min_dt is None or max_dt is None:
            st.warning("No rows found for this device.")
            st.stop()
        st.session_state["ub_date_bounds"] = (min_dt, max_dt)
        st.session_state["ub_loaded_device"] = selected_device

    bounds = st.session_state.get("ub_date_bounds")
    if not bounds:
        st.info("Click 'Load date range for this device' to continue.")
        st.stop()

    all_dates = pd.Series([bounds[0], bounds[1]])
    ...
    with tc1:
        start_time = st.time_input("Start time", value=dtime(0, 0), key="ub_start_time")
    with tc2:
        end_time   = st.time_input("End time",   value=dtime(23, 59), key="ub_end_time")
    with tc3:
        min_req_content = st.number_input(
            "Min requests per content", min_value=1, max_value=1000, value=1, step=1,
            key="ub_min_req",
        )

    # ── Load & enrich ─────────────────────────────────────────
    run_dash_btn = st.button("▶️ Run Dashboard", type="primary", key="ub_run_btn")

    if not run_dash_btn and st.session_state.get("ub_window_df") is None:
        st.info("Configure settings above and click **Run Dashboard**.")
        st.stop()

    if run_dash_btn:
        _rbar = st.progress(0, text="Starting ...")
        _rph  = st.empty()

        # Step 1 — DuckDB query
        _rbar.progress(5, text="📂 Step 1/4 — Querying parquet files ...")
        _stages_run = [
            (5,  "📂 Step 1/4 — Opening parquet files"),
            (20, "📂 Step 1/4 — Filtering by device_id and date range"),
            (45, "📂 Step 1/4 — Reading matched rows"),
            (65, "📂 Step 1/4 — Parsing query string fields"),
            (80, "📂 Step 1/4 — Transferring to DataFrame"),
        ]
        _raw_df = ub_staged_progress(_rbar, _rph, _stages_run,
                                      lambda: ub_load_device_all(pq_glob, selected_device, cm))
        if _raw_df is None or _raw_df.empty:
            st.warning("No data found for this device.")
            st.stop()

        # Step 2 — Enrich once per device
        _rbar.progress(60, text="🔧 Step 2/4 — Enriching data (timestamps, channels, quality) ...")
        _rph.caption("⏳ Converting timestamps and decoding URL fields")
        device_df = ub_enrich(_raw_df)
        if device_df.empty:
            st.warning("No data found after enrichment.")
            st.stop()

        # Filter selected date range in memory
        device_df = device_df[
        ...
        n_rows_enrich = len(device_df)
        _rbar.progress(75, text=f"🔗 Step 3/4 — Detecting sessions across {n_rows_enrich:,} rows ...")
        ...
        # Step 4 — Watch time estimation
        _rbar.progress(90, text="⏱️ Step 4/4 — Estimating watch minutes ...")
        ...
        _rbar.progress(100, text="✅ All done!")
        time.sleep(0.3)
        _rbar.empty()
        _rph.empty()
        ...
        st.session_state["ub_run_params"] = f"{selected_device}|{start_date}|{end_date}"
    ...
    with dl4:
        st.download_button(
            "📄 Export charts PDF",
            data=pdf_bytes,
            file_name=f"behavior_{selected_device}_{date_label}.pdf",
            mime="application/pdf", key="ub_dl_pdf",
        )
