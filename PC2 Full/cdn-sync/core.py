...
def list_source_date_range(src: dict, start: datetime, end: datetime) -> dict:
    """List S3 objects for a source across a date range."""
    # Both mmdd and mmddx have real MM/DD/ folders in S3; list in parallel
    all_objs, futures = {}, {}
    with ThreadPoolExecutor(max_workers=config.LIST_WORKERS) as pool:
        cur = start
        while cur <= end:
            fut = pool.submit(list_prefix,
                f"{src['s3_prefix']}{cur:%m}/{cur:%d}/")
            futures[fut] = cur.strftime("%m/%d")
            cur += timedelta(days=1)
        for fut in as_completed(futures):
            all_objs.update(fut.result())
    return all_objs
