...
def read_excel(xlsx_path: Path) -> tuple[dict, str, list[dict]]:
    ...
    Excludes the most-recent (likely partial) date automatically.
    """
    ...
    # Drop the latest (likely partial) date
    if data_rows:
        latest = max(r["date"] for r in data_rows)
        data_rows = [r for r in data_rows if r["date"] != latest]
