...
## v4 Persistent speed layer

This version adds:

- Persistent Global Behavior disk cache under `.pdash_cache/global_bundles`
- Lightweight file metadata database under `.pdash_cache/metadata.sqlite`
- Cache keys include selected files, file size, file modified time, date range, column mapping, day-part mode, and preload size
- Restarting Streamlit can reuse heavy Global Behavior calculations without recomputing
- `Reload Global Cache` clears persisted Global Behavior cache and recomputes from parquet

Environment knobs:

```bash
PDASH_CACHE_DIR=.pdash_cache
PDASH_DISK_CACHE=1
PDASH_DISK_CACHE_MAX_AGE_SECONDS=604800
```
