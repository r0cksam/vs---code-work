...
# pDash_combined.py
# ===================================================================
# Merged version of pDash_stable_v14 and pDashV15 with all features:
# - Lake + Browse + Manual folder selection
# - Persistent DuckDB cache (file inventory, column profiles, unique values)
# - Performance fixes (no sleep, DuckDB‑first unique values, cardinality cap)
# - IP & Geo Intelligence tab (requires requests)
# - Watch Hour Analysis tab (deduplicated chunks, configurable chunk duration)
# ===================================================================
...
PDASH_CACHE_VERSION = "v15_combined"
...
def gb_get_coverage(parquet_glob: tuple, col_map: dict, start_date: str, end_date: str) -> dict:
    con = ub_get_conn()
    ...
    FROM base
    """
    row = con.execute(query, [list(parquet_glob), start_date, end_date]).fetchone()
    ...

# ----- Lake mode helpers (from v14) -----
@st.cache_data(show_spinner=False, ttl=900)
def collect_files_lake(lake_root: str, start_date: str, end_date: str) -> list:
    import datetime as _dt
    import calendar as _cal   # moved outside loops
    root = Path(lake_root.strip())
    if not root.is_dir():
        return []
    try:
    ...
    for year_dir in sorted(root.iterdir()):
        if not year_dir.is_dir():
            continue
        yr_name = year_dir.name.replace("year=", "")
        try:
            yr = int(yr_name)
        except ValueError:
            continue
        if yr < start.year or yr > end.year:
            continue
        for month_dir in sorted(year_dir.iterdir()):
            if not month_dir.is_dir():
                continue
            mo_name = month_dir.name.replace("month=", "")
            try:
                mo = int(mo_name)
            except ValueError:
                continue
            month_start = _dt.date(yr, mo, 1)
            month_end   = _dt.date(yr, mo, _cal.monthrange(yr, mo)[1])
            if month_end < start or month_start > end:
                continue
            for day_dir in sorted(month_dir.iterdir()):
                if not day_dir.is_dir():
                    continue
                dy_name = day_dir.name.replace("day=", "")
                try:
                    dy = int(dy_name)
                    folder_date = _dt.date(yr, mo, dy)
...
# ===== Core data helpers (merged from both) =====
@st.cache_data(show_spinner=False)
...
# ===== Query String helpers (unchanged) =====
...
# ===== User Behavior Dashboard helpers (from v15 with guard) =====
...
def ub_get_device_ids(parquet_glob: tuple, qs_col: str) -> list:
    """parquet_glob must be a tuple of file paths (hashable)"""
    con = ub_get_conn()
    # Parameterized query: pass the list as a parameter
    query = f"""
    ...
    """
    try:
        df = con.execute(query, [list(parquet_glob)]).df()
        return df["device_id"].astype(str).tolist()
    except Exception as e:
        return []

@st.cache_data(show_spinner="Loading device data ...", ttl=600)
def ub_load_device(parquet_glob: tuple, device_id: str, start_date: str, end_date: str, col_map: dict) -> pd.DataFrame:
    con = ub_get_conn()
    qs   = _cm(col_map, "queryStr")
    ts   = _cm(col_map, "reqTimeSec")
    ...
    """
    try:
        return con.execute(query, [list(parquet_glob), device_id, start_date, end_date]).df()
    except Exception as e:
...
# ===== Global Behavior helpers (from v15 with three‑state cache) =====
@st.cache_data(show_spinner="Loading global behavior coverage ...", ttl=900)
...
# ===== IP & Geo Intelligence helpers (from v15) =====
@st.cache_data(show_spinner=False, ttl=900)
def gda_load_raw(parquet_glob: list, col_map: dict, start_date: str, end_date: str, sample_limit: int = 2_000_000) -> pd.DataFrame:
    ...
    try:
        ...
        if "reqTimeSec" in df.columns:
            df["event_time"] = pd.to_datetime(pd.to_numeric(df["reqTimeSec"], errors="coerce"), unit="s", errors="coerce")
        return df
    except Exception as e:
...
def gda_device_content_table(df: pd.DataFrame, top_paths: int = 20) -> pd.DataFrame:
    ...
    grp = grp.groupby("device_id", sort=False).head(top_paths).reset_index(drop=True)
    return grp
...
# ===== NEW: Watch Hour Analysis Tab =====
def watch_hour_analysis(parquet_glob: tuple, col_map: dict, start_date: str, end_date: str, chunk_duration_hours: float, channel_map: dict):
    con = ub_get_conn()
    # ... (inside the query, replace read_parquet({parquet_glob!r}) with read_parquet(?) and parameter)
    query = f"""
    WITH base AS (
        SELECT {ip} AS cliIP, {path} AS reqPath, {sc} AS statusCode, {ts} AS reqTimeSec, {stream_id_expr} AS stream_id
        FROM read_parquet(?, union_by_name=true)
        WHERE {path} LIKE '%.ts' AND {sc} = '200'
          AND to_timestamp(TRY_CAST({ts} AS BIGINT))::DATE BETWEEN ? AND ?
    )
    ...
    """
    channel_df = con.execute(query, [list(parquet_glob), start_date, end_date]).df()
    # similarly for user_query
# ===== Sidebar: Combined folder selection (Lake + Browse + Manual) =====
with st.sidebar:
    st.title("🗂️ Parquet Explorer")
    st.markdown("---")
    mode = st.radio(
        "Folder selection mode",
        ["🗄️ Lake (date-partitioned)", "🔍 Browse from root", "✏️ Type paths manually"],
        horizontal=False,
        key="folder_mode",
    )
    st.markdown("---")

    if mode == "🗄️ Lake (date-partitioned)":
        st.markdown("**Lake root folder**")
        st.caption("Point at the lake root that contains `year=YYYY/month=MM/day=DD/` partitions.")
        lake_root_input = st.text_input(
            "Lake root",
            value=st.session_state.get("lake_root", ""),
            placeholder=r"e.g. D:\Veto Logs Backup\05 Veto Logs\lake",
            key="lake_root_input",
        )
        st.session_state["lake_root"] = lake_root_input.strip()
        import datetime as _dt_sidebar
        _today = _dt_sidebar.date.today()
        _default_start = _today - _dt_sidebar.timedelta(days=6)
        lake_dates = st.date_input(
            "Date range to load",
            value=(
                st.session_state.get("lake_start", _default_start),
                st.session_state.get("lake_end",   _today),
            ),
            key="lake_date_input",
        )
        if isinstance(lake_dates, (list, tuple)) and len(lake_dates) == 2:
            lake_start, lake_end = lake_dates
        else:
            lake_start, lake_end = _default_start, _today
        st.session_state["lake_start"] = lake_start
        st.session_state["lake_end"]   = lake_end
        if st.button("🗄️ Load lake", type="primary", key="btn_lake_load"):
            collect_files_lake.clear()
            st.rerun()
        if lake_root_input.strip():
            lake_root_path = Path(lake_root_input.strip())
            if not lake_root_path.is_dir():
                st.error("Lake root folder not found.")
            else:
                with st.spinner("Resolving lake partitions …"):
                    lake_files = collect_files_lake(
                        lake_root_input.strip(),
                        str(lake_start),
                        str(lake_end),
                    )
                if lake_files:
                    day_dirs = sorted({str(Path(f).parent) for f in lake_files})
                    st.session_state.selected_folders = day_dirs
                    st.success(
                        f"✅  {len(lake_files):,} file(s) across {len(day_dirs)} day-partition(s) "
                        f"({lake_start} → {lake_end})"
                    )
                else:
                    st.session_state.selected_folders = []
                    st.warning(
                        "No parquet files found for this date range. "
                        "Check the lake root and that the partitions exist."
                    )
        else:
            st.session_state.selected_folders = []

    elif mode == "🔍 Browse from root":
        ...
        if st.button("🔍 Scan", type="primary", key="btn_scan") or (root_input.strip() and root_changed):
        ...
        if results:
            ...
            for r in results:
                ...
                if st.button(label, key=f"fld_{r['path']}"):
                    if is_selected:
                        st.session_state.selected_folders = [x for x in st.session_state.selected_folders if x != r["path"]]
                    else:
    ...
    st.markdown("---")
    st.caption("Tabs: 1 Columns | 2 Unique | 3 Filter | 4 QSA | 5 User Behavior | 6 Global Behavior | 7 IP & Geo | 8 Watch Hours")
...
if not valid_folders:
    ...
    st.markdown("""
    **Three ways to add folders:**
    - **Lake (date‑partitioned)** — point at a root with `year=*/month=*/day=*/` structure.
    - **Browse from root** — type a root path, click Scan, then click any folder.
    - **Type paths manually** — paste exact folder paths one by one.

    **Features:**
    - 📋 Columns + completeness profile
    - 🔍 Unique values explorer
    - 🎯 Filter & export data
    - 🔎 Query String Analyzer (device/session IDs)
    - 📺 User Behavior Dashboard (per device)
    - 🌐 Global Behavior (day‑part, stickiness, retention, switching)
    - 🛰️ IP & Geo Intelligence (IP‑device relationships)
    - ⏱️ Watch Hour Analysis (deduplicated .ts chunks, channel/user watch hours)
    """)
    st.stop()
...
# Show cache panel only if persistent cache is available
with st.sidebar:
...
# ══════════════════════════════════════════════
# TAB 8 — Watch Hour Analysis
# ══════════════════════════════════════════════
with tab8:
    st.subheader("⏱️ Watch Hour Analysis (Deduplicated Segments)")
    st.caption(
        "Estimates total watch hours by counting **unique** `.ts` chunk deliveries (deduplicated by client IP, channel, and exact path) "
        "and multiplying by a fixed chunk duration. Only successful requests (statusCode=200) are considered."
    )

    # Use the same column mapping as User Behavior
    cm = st.session_state.ub_col_map
    qs_col  = cm.get("queryStr", "")
    ts_col  = cm.get("reqTimeSec", "")
    path_col = cm.get("reqPath", "")
    ip_col   = cm.get("cliIP", "")
    sc_col   = cm.get("statusCode", "")

    if not qs_col or not ts_col or not path_col or not ip_col:
        st.warning(
            "Please configure Query string, Timestamp, Request path, and Client IP columns "
            "in the User Behavior tab's column mapping first."
        )
        st.stop()

    # Date range picker for this tab (independent of lake range if any)
    wh_start = st.date_input("Start date", value=pd.Timestamp("today") - pd.Timedelta(days=7), key="wh_start")
    wh_end   = st.date_input("End date",   value=pd.Timestamp("today"), key="wh_end")

    # Chunk duration (default 6 seconds = 6/3600 hours)
    chunk_duration_sec = st.number_input(
        "Chunk duration (seconds)", min_value=1, max_value=30, value=6, step=1,
        help="Duration of each .ts segment. Typical values: 2, 4, 6, 10 seconds."
    )
    chunk_duration_hours = chunk_duration_sec / 3600.0

    # Channel mapping editor
    with st.expander("✏️ Edit channel name mapping (optional)"):
        st.caption("Map stream IDs (extracted from reqPath) to friendly channel names.")
        default_map = {
        ...
        channel_map_str = st.text_area(
            "Mapping (one per line: stream_id=Channel Name)",
            value="\n".join(f"{k}={v}" for k, v in default_map.items()),
            height=200,
            help="Format: stream_id=Channel Name"
        )
        channel_map = {}
        for line in channel_map_str.strip().split("\n"):
            if "=" in line:
                k, v = line.split("=", 1)
                channel_map[k.strip()] = v.strip()

    if st.button("🚀 Run Watch Hour Analysis", type="primary", key="wh_run"):
        with st.spinner("Analyzing .ts chunks (this may take a while) ..."):
            parquet_list = [str(f) for f in collect_files(valid_folders)]
            if not parquet_list:
                st.error("No parquet files found.")
            else:
                try:
                    channel_df, user_df = watch_hour_analysis(
                        parquet_list, cm, str(wh_start), str(wh_end),
                        chunk_duration_hours, channel_map
                    )
                    st.session_state["wh_channel_df"] = channel_df
                    st.session_state["wh_user_df"] = user_df
                except Exception as e:
                    st.error(f"Analysis failed: {e}")

    if "wh_channel_df" in st.session_state:
        channel_df = st.session_state["wh_channel_df"]
        user_df    = st.session_state["wh_user_df"]

        if channel_df.empty:
            st.info("No .ts segments found for the selected date range.")
        else:
            st.markdown("### 📊 Global Channel Watch Hours")
            st.dataframe(channel_df, use_container_width=True, hide_index=True)

            fig = px.bar(
                channel_df.head(15),
                x="watch_hours", y="channel_name", orientation="h",
                title="Top 15 Channels by Estimated Watch Hours",
            ...
            st.download_button(
                "📥 Download Channel Report CSV",
                data=channel_df.to_csv(index=False).encode("utf-8"),
                file_name=f"channel_watch_hours_{wh_start}_{wh_end}.csv",
                mime="text/csv",
                key="wh_dl_channel"
            )

            st.markdown("---")
            st.markdown("### 👥 Top Users per Channel")
            ...
            st.download_button(
                ...
                file_name=f"user_watch_hours_{wh_start}_{wh_end}.csv",
                mime="text/csv",
                key="wh_dl_user"
            )

            total_hours = channel_df["watch_hours"].sum()
            total_devices = user_df["cliIP"].nunique()
            st.markdown(f"**Total estimated watch hours:** {total_hours:,.2f} hrs")
            st.markdown(f"**Unique client IPs (viewers):** {total_devices:,}")