...
def gb_behavior_cte(parquet_glob: list, col_map: dict, start_date: str, end_date: str) -> str:
    qs   = _cm(col_map, "queryStr")
    ts   = _cm(col_map, "reqTimeSec")
    path = _cm(col_map, "reqPath")
    ua   = _cm(col_map, "UA")
    ...
    enriched AS (
        SELECT
            *,
            COALESCE(NULLIF(channel_qs, ''), NULLIF(regexp_extract(reqPath, '(vglive-sk-\d+)', 1), ''), 'Unknown') AS channel_name,
...
def gb_get_daypart_top(parquet_glob: list, col_map: dict, start_date: str, end_date: str, entity: str, top_n: int) -> pd.DataFrame:
    con = get_db_conn()
    entity_expr = "channel_name" if entity == "Channel" else "content_label"
    query = gb_behavior_cte(parquet_glob, col_map, start_date, end_date) + f"""
    SELECT *
    FROM (
        SELECT
            day_part,
...
def gb_get_stickiness(parquet_glob: list, col_map: dict, start_date: str, end_date: str, entity: str, top_n: int) -> pd.DataFrame:
    con = get_db_conn()
    entity_expr = "channel_name" if entity == "Channel" else "content_label"
    query = gb_behavior_cte(parquet_glob, col_map, start_date, end_date) + f"""
    , seq AS (
...
def gb_get_retention_buckets(parquet_glob: list, col_map: dict, start_date: str, end_date: str, entity: str, top_n: int) -> pd.DataFrame:
    con = get_db_conn()
    entity_expr = "channel_name" if entity == "Channel" else "content_label"
    query = gb_behavior_cte(parquet_glob, col_map, start_date, end_date) + f"""
    , seq AS (
...
def gb_get_switching(parquet_glob: list, col_map: dict, start_date: str, end_date: str, top_n: int) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    con = get_db_conn()
    cte = gb_behavior_cte(parquet_glob, col_map, start_date, end_date)
    trans_query = cte + f"""
    , seq AS (
        SELECT
            *,
            LEAD(channel_name) OVER (PARTITION BY device_id, session_id ORDER BY event_time) AS next_channel,
            LEAD(event_time) OVER (PARTITION BY device_id, session_id ORDER BY event_time) AS next_event_time
        FROM enriched
    )
    SELECT
    ...
    FROM seq
    WHERE next_channel IS NOT NULL AND next_channel <> channel_name
    GROUP BY 1,2
    ...
    SELECT
        channel_name,
        COUNT(*) AS events,
        ROUND(100.0 * AVG(CASE WHEN next_channel IS NOT NULL AND next_channel <> channel_name THEN 1 ELSE 0 END), 2) AS switch_away_pct
    FROM seq
    GROUP BY 1
    HAVING COUNT(*) >= 20
    ...
    , seq AS (
        SELECT
            *,
            LAG(channel_name) OVER (PARTITION BY device_id, session_id ORDER BY event_time) AS prev_channel
        FROM enriched
    ),
    session_switches AS (
        SELECT
            session_id,
            COUNT(*) FILTER (WHERE prev_channel IS NOT NULL AND prev_channel <> channel_name) AS switches
        FROM seq
        GROUP BY 1
    )
    SELECT
...
with tab6:
    ...
    st.caption("Behavior analytics are calculated only from rows that contain session metadata. Traffic coverage is shown separately so you can judge how representative the behavior layer is.")
    ...
    with g3:
        gb_show_overall = st.checkbox("Show overall coverage baseline", value=True, key="gb_show_overall")

    all_start, all_end = "1970-01-01", "2100-01-01"
    gb_dates = st.date_input("Date range", value=(pd.to_datetime("today").date() - pd.Timedelta(days=6), pd.to_datetime("today").date()), key="gb_date_range")
    gb_start, gb_end = gb_dates if isinstance(gb_dates, tuple) and len(gb_dates) == 2 else (pd.to_datetime("today").date(), pd.to_datetime("today").date())
    gb_start_s, gb_end_s = str(gb_start), str(gb_end)

    cov = gb_get_coverage(gb_glob, gb_cm, gb_start_s, gb_end_s)
    overall_cov = gb_get_coverage(gb_glob, gb_cm, all_start, all_end) if gb_show_overall else None
    ...
    with sec1:
        day_df = gb_get_daypart_top(gb_glob, gb_cm, gb_start_s, gb_end_s, gb_entity, int(gb_top_n))
        if day_df.empty:
    ...
    with sec2:
        sticky_df = gb_get_stickiness(gb_glob, gb_cm, gb_start_s, gb_end_s, gb_entity, max(int(gb_top_n) * 3, 15))
        if sticky_df.empty:
    ...
    with sec3:
        ret_df = gb_get_retention_buckets(gb_glob, gb_cm, gb_start_s, gb_end_s, gb_entity, int(gb_top_n))
        if ret_df.empty:
    ...
    with sec4:
        trans_df, rate_df, sess_bucket_df = gb_get_switching(gb_glob, gb_cm, gb_start_s, gb_end_s, max(int(gb_top_n) * 2, 10))
