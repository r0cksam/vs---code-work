...
def collect_files(folders: list) -> list:
    """Return sorted parquet files for selected folders (recursive), cached to avoid repeated disk scans.

    Uses rglob so that Hive-partitioned lake folders like:
        lake/year=2026/month=04/day=29/src01_0.parquet
    are discovered correctly when the user selects the lake root folder.
    """
    files = []
    for f in folders:
        p = Path(str(f).strip())
        ...
        if p.is_dir():
            try:
                files.extend(sorted(p.rglob("*.parquet")))   # rglob: recursive, finds nested lake files
...
# ─────────────────────────────────────────────
# Hive-lake detection & DuckDB reader builder
# ─────────────────────────────────────────────

# Partition columns injected by the lake script — exclude from user-visible schema.
_LAKE_PARTITION_COLS = frozenset({"year", "month", "day"})


def _is_hive_lake(folder: Path) -> bool:
    """Return True if the folder looks like a Hive-partitioned lake (has year=* subdirs)."""
    try:
        return any(
            d.is_dir() and (d.name.startswith("year=") or d.name.startswith("month="))
...
@st.cache_data(show_spinner=False, ttl=900)
def _detect_lake_folders(folder_key: str) -> dict:
    """Return {folder_path: is_hive} for all selected folders. Cached per folder_key."""
    result = {}
    for f in folder_key.split("|"):
        p = Path(f.strip())
        result[f] = _is_hive_lake(p)
    return result


def build_duckdb_reader(
    files_or_folders,
    start_date: Optional[str] = None,
    end_date: Optional[str] = None,
) -> str:
    """
    Build the SQL read_parquet(...) expression for use inside DuckDB queries.

    Flat folders  →  read_parquet([list of files], union_by_name=true)
    Hive lake     →  read_parquet(['lake/**/*.parquet'], hive_partitioning=true, union_by_name=true)
                     + automatic partition pruning via year/month/day WHERE clause.

    Partition pruning massively speeds up date-range queries on the lake because
    DuckDB skips entire year=/month=/day= directories without opening any files.

    Parameters
    ----------
    files_or_folders : list[str | Path]
        Either a list of parquet file paths (from collect_files), or a list of
        folder root paths (for Hive lakes — pass valid_folders directly).
    start_date, end_date : str  "YYYY-MM-DD"
        Optional date range.  When provided and a Hive lake is detected, adds
        year/month/day partition filters for DuckDB partition pruning.

    Returns
    -------
    str — a SQL fragment like:
        read_parquet([...], union_by_name=true)
      or
        read_parquet(['lake/**/*.parquet'], hive_partitioning=true, union_by_name=true)
    """
    if not files_or_folders:
        return "read_parquet([], union_by_name=true)"

    # Determine if inputs are folders (lake mode) or individual files (flat mode)
    paths = [Path(str(x)) for x in files_or_folders]

    # Check if any path is a lake root (has year= subdirs)
    lake_roots = []
    flat_files  = []
    for p in paths:
        if p.is_dir() and _is_hive_lake(p):
            lake_roots.append(p)
        elif p.is_dir():
            flat_files.extend(p.rglob("*.parquet"))
        else:
            flat_files.append(p)  # it's already an individual file path

    parts = []

    if lake_roots:
        # Build one glob pattern per lake root
        globs = [f"{root.as_posix()}/**/*.parquet" for root in lake_roots]
        globs_repr = repr(globs) if len(globs) > 1 else repr(globs[0])
        parts.append(f"read_parquet({globs_repr}, hive_partitioning=true, union_by_name=true)")

    if flat_files:
        flat_strs = [str(f) for f in sorted(set(flat_files))]
        parts.append(f"read_parquet({flat_strs!r}, union_by_name=true)")

    if len(parts) == 1:
        reader = parts[0]
    elif len(parts) > 1:
        # Combine with UNION ALL at the query level — caller wraps this
        reader = parts[0]   # primary; caller should handle multi-source separately
    else:
        reader = "read_parquet([], union_by_name=true)"

    return reader


def build_partition_filter(start_date: Optional[str], end_date: Optional[str], is_hive: bool) -> str:
    """
    Build a SQL WHERE fragment that uses Hive partition columns (year, month, day)
    for coarse filtering when the data is a Hive lake.

    DuckDB pushes these predicates down to the partition level — entire directories
    are skipped without opening any files.

    Returns empty string if is_hive=False or dates are not provided.
    """
    if not is_hive or not start_date or not end_date:
        return ""
    try:
        s = datetime.strptime(start_date, "%Y-%m-%d")
        e = datetime.strptime(end_date,   "%Y-%m-%d")
        # Build year range — coarse filter (DuckDB skips irrelevant year= dirs)
...
def load_schema(folder_key: str) -> tuple:
    ...
    Lake partition columns (year, month, day) injected by the lake script are
    excluded from the user-visible schema to avoid confusion — they are string
    duplicates of reqTimeSec and not useful as filter columns.
    """
    ...
    for f in files:
        try:
            ...
            for name in schema.names:
                if name in _LAKE_PARTITION_COLS:
                    continue   # skip injected lake partition columns
...
def column_completeness_profile(folder_key: str, profile_cols: list, mode: str = "Exact selected columns", sample_rows: int = 100000) -> pd.DataFrame:
    ...
    con = ub_get_conn()
    reader = build_duckdb_reader(folders)
...
def ub_get_device_ids(parquet_glob: str, qs_col: str) -> list:
    con    = ub_get_conn()
    reader = build_duckdb_reader([parquet_glob] if isinstance(parquet_glob, str) else parquet_glob)
    query = f"""
...
def ub_load_device(parquet_glob: str, device_id: str, start_date: str, end_date: str, col_map: dict) -> pd.DataFrame:
    ...
    reader   = build_duckdb_reader([parquet_glob] if isinstance(parquet_glob, str) else parquet_glob)
    part_flt = build_partition_filter(start_date, end_date, "hive_partitioning" in reader)
    query = f"""
    SELECT
        {qs}            AS queryStr,
        {ts}            AS reqTimeSec,
...
def gb_get_coverage(parquet_glob: list, col_map: dict, start_date: str, end_date: str) -> dict:
    ...
    qs   = _cm(col_map, "queryStr")
    ts   = _cm(col_map, "reqTimeSec")
    reader     = build_duckdb_reader(parquet_glob)
    part_flt   = build_partition_filter(start_date, end_date, "hive_partitioning" in reader)
    query = f"""
    WITH base AS (
...
with st.sidebar:
    ...
    if mode == "🔍 Browse from root":
        ...
        for f in manual:
            ...
            elif not any(p.rglob("*.parquet")):   # rglob: works for both flat and lake folders
                st.warning(f"No .parquet files found (including subdirectories): {f}")
            else:
                valid.append(f)
...
for f in st.session_state.selected_folders:
    f = str(f).strip()
    p = Path(f)
    if p.is_dir() and any(p.rglob("*.parquet")):   # rglob: accepts lake root folders
...
with st.expander(f"📂 {n_fol} active folder(s)", expanded=False):
    for f in valid_folders:
        fpath = Path(f)
        cnt   = sum(1 for _ in fpath.rglob("*.parquet"))   # rglob: counts nested lake files
