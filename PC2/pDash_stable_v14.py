...
def collect_files(folders: list) -> list:
    ...
    Supports both flat folders (*.parquet directly inside) and lake-partitioned folders
    (year=YYYY/month=MM/day=DD/**/*.parquet).  A folder is treated as a lake partition
    leaf when it contains .parquet files but has a parent whose name matches month=* or
    day=* — i.e. it was selected by collect_files_lake already.  Plain folders that hold
    no .parquet at the top level are scanned recursively one extra level so that pointing
    at the lake root also works (though date-range pruning is better done via lake mode).
    """
    files = []
    for f in folders:
        p = Path(str(f).strip())
        if not p.is_dir():
            continue
        try:
            direct = sorted(p.glob("*.parquet"))
            if direct:
                files.extend(direct)
            else:
                # Recurse into partitioned sub-tree (lake root pointed directly)
...
def collect_files_lake(lake_root: str, start_date: str, end_date: str) -> list:
    """Collect parquet files from a date-partitioned lake for a given date range.

    Expects structure:  <lake_root>/year=YYYY/month=MM/day=DD/*.parquet
    Only day-folders whose date falls within [start_date, end_date] are included.
    Falls back gracefully if the folder structure doesn't match expectations.
    """
    ...
    for year_dir in sorted(root.iterdir()):
        if not year_dir.is_dir():
            continue
        # Support both  year=YYYY  and  YYYY  naming
        ...
        for month_dir in sorted(year_dir.iterdir()):
            ...
            try:
                ...
                continue
            # Quick month-level prune
            # A month is in range if any day of it could overlap [start, end]
...
@st.cache_data(show_spinner="📋 Reading column schema from parquet files ...", ttl=1800)
...
def count_stats(folder_key: str):
    ...
    # Use DuckDB for parallel row counting — much faster than a Python
    # loop over pq.read_metadata() when there are many files.
    if DUCKDB_OK:
        try:
            con = ub_get_conn()
            ...
            log_warning("DuckDB count failed, falling back to metadata loop", e)
    # Fallback: pyarrow metadata (no DuckDB)
    total_rows = 0
    for f in files:
        try:
...
def _make_parquet_source(parquet_glob: list) -> str:
    """Build the read_parquet(...) SQL fragment for a list of file paths.

    If every file lives inside a Hive-partitioned lake tree
    (i.e. its grandparent folder name starts with 'month=' or 'year='),
    we swap to a single glob pattern with hive_partitioning=true so
    DuckDB can prune year/month/day folders at the metadata level
    instead of opening every file.

    Falls back to the plain list form for non-lake (flat folder) data.
    """
    if not parquet_glob:
        return "read_parquet([], union_by_name=true)"

    # Detect lake layout: check first few files for hive parent dirs
    sample = parquet_glob[:5]
    hive_hits = 0
    lake_roots = set()
    for fp in sample:
        p = Path(fp)
        # day-dir parent: day=DD, grandparent: month=MM, great-grandparent: year=YYYY
        if p.parent.name.startswith("day=") and p.parent.parent.name.startswith("month="):
            hive_hits += 1
            lake_roots.add(str(p.parent.parent.parent.parent))  # lake root

    if hive_hits == len(sample) and len(lake_roots) == 1:
        lake_root = list(lake_roots)[0].replace("\\", "/").replace("\\", "/")
        # Use forward slashes for DuckDB glob even on Windows
        lake_root_fwd = lake_root.replace("\\", "/")
        return (
            f"read_parquet('{lake_root_fwd}/year=*/month=*/day=*/*.parquet', "
            f"union_by_name=true, hive_partitioning=true)"
        )

    # Non-lake: plain list (safe for any mix of files)
    return f"read_parquet({parquet_glob!r}, union_by_name=true)"
...
def unique_values(folder_key: str, column: str) -> pd.DataFrame:
    ...
    # Fast path: single DuckDB aggregation (parallel I/O, no Python loop)
    if DUCKDB_OK:
        try:
            con = ub_get_conn()
            parquet_list = [str(f) for f in files]
            qcol = _dq(column)
            bar = st.progress(50, text=f"Counting unique values with DuckDB ...")
            df = con.execute(f"""
                SELECT
                    COALESCE(TRY_CAST({qcol} AS VARCHAR), '(null)') AS value,
                    COUNT(*) AS count
...
def full_filtered_value_counts(
    ...
    Fast path: single DuckDB query with WHERE pushdown for simple equality
    filters (no dual). Falls back to the PyArrow batch loop for dual filters
    or when DuckDB is unavailable.
    """
    files = collect_files(folder_key.split("|"))
    if not files:
        return pd.DataFrame(columns=[target_col, "count"])

    # ── DuckDB fast path (simple filters only, no dual) ───────────────
    has_dual = dual and (dual.get("vals_a") or dual.get("vals_b"))
    if DUCKDB_OK and not has_dual:
        try:
            con = ub_get_conn()
            parquet_list = [str(f) for f in files]
            qcol = _dq(target_col)
            ...
            for col, vals in (filters or {}).items():
                ...
                where_clauses.append(f"TRY_CAST({_dq(col)} AS VARCHAR) IN ({placeholders})")
                params.extend([str(v) for v in vals])

            where_sql = " AND ".join(where_clauses)
            bar = st.progress(50, text=f"{progress_label} — running DuckDB aggregation ...")
            df = con.execute(f"""
                SELECT
                    COALESCE(NULLIF(TRY_CAST({qcol} AS VARCHAR), ''), '(blank)') AS {_dq(target_col)},
                    COUNT(*) AS count
                FROM read_parquet({parquet_list!r}, union_by_name=true)
                WHERE {where_sql}
                GROUP BY 1
            ...
            if include_other and len(df) > int(top_n):
                head = df.head(int(top_n)).copy()
                other_count = int(df.iloc[int(top_n):]["count"].sum())
                head = pd.concat([head, pd.DataFrame([{target_col: "Other", "count": other_count}])], ignore_index=True)
                df = head
            else:
                df = df.head(int(top_n)).copy()
            df["% of filtered rows"] = (df["count"] / total_count * 100).round(2) if total_count else 0
            return df.reset_index(drop=True)
        except Exception as e:
            log_warning("DuckDB full_filtered_value_counts failed, falling back to PyArrow", e)

    # ── PyArrow fallback (dual filters or no DuckDB) ──────────────────
    file_cols = get_file_columns(folder_key)
    total = len(files)
    counter = {}
    needed_cols = list(dict.fromkeys(
        ...
        + [dual.get("col_a") if dual else None, dual.get("col_b") if dual else None]
    ))
    needed_cols = [c for c in needed_cols if c]
...
@st.cache_resource
def ub_get_conn():
    """Shared DuckDB connection for UB / general queries."""
...
@st.cache_resource
def gb_get_conn():
    """Separate DuckDB connection for heavy Global Behavior analytics.

    Kept separate from ub_get_conn so that a long GB scan never blocks
    a UB device lookup and vice-versa (DuckDB connections are not
    thread-safe when shared across concurrent queries).
    """
    con = duckdb.connect(database=":memory:")
...
def ub_enrich(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Vectorized channel normalisation — avoids a Python-level row loop.
    # Build the suffix-strip regex once from DEVICE_SUFFIX_TOKENS; this
    # mirrors what normalize_channel_name_smart() does but runs in C via pandas.
    _suffix_pat = "|".join(sorted(DEVICE_SUFFIX_TOKENS, key=len, reverse=True))
    _re_mod     = re  # local alias for clarity inside vectorised block
    _strip_re   = rf"(_(?:{_suffix_pat}))+$"
    _raw_s      = out["channel_name_raw"].fillna("Unknown").astype(str)
    _channel_v  = (
        _raw_s
        .str.strip()
        .str.replace(r"\s*_\s*", "_", regex=True)
        .str.replace(_strip_re, "", regex=True)
        .str.strip("_ ")
        .str.casefold()
        .replace({"": "unknown", "nan": "unknown", "none": "unknown", "null": "unknown", "(null)": "unknown"})
        .fillna("unknown")
    )
    out["channel_name"] = _channel_v.replace("", "Unknown")
...
def gb_get_coverage(parquet_glob: list, col_map: dict, start_date: str, end_date: str) -> dict:
    ...
    WITH base AS (
        ...
        FROM {_make_parquet_source(parquet_glob)}
...
with st.sidebar:
    ...
    if mode == "🗄️ Lake (date-partitioned)":
        ...
        st.caption(
            "Point at the lake root that contains `year=YYYY/month=MM/day=DD/` partitions. "
            "Only the day-folders inside your date range will be loaded — no wasted I/O."
        )
            ...
            if not lake_root_path.is_dir():
                ...
                if lake_files:
                    ...
                    st.warning(
                        ...
                        "Check the lake root and that 03.py has written the partitions."
                    )
        else:
            st.session_state.selected_folders = []
...
for f in st.session_state.selected_folders:
    f = str(f).strip()
    p = Path(f)
    if p.is_dir() and (list(p.glob("*.parquet")) or list(p.rglob("*.parquet")))[:1]:
...
with st.expander(f"📂 {n_fol} active folder(s)", expanded=False):
    for f in valid_folders:
        fpath = Path(f)
        cnt_direct = list(fpath.glob("*.parquet"))
        cnt = len(cnt_direct) if cnt_direct else sum(1 for _ in fpath.rglob("*.parquet"))
...
with tab6:
    ...
    def _gb_cache_get(name):
        item = st.session_state.get(name)
        if not item:
            return None
        key, payload = item
