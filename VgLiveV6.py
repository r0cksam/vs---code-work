...
def calculate_watch_hours(file_path):
    ...
    # 🚨 ADDED: Loading 'reqTimeSec' to track the timeline window
    ...
    try:
        ...
        # 🚨 NEW TIMELINE LOGIC: Find the log's exact time boundaries
        # Convert numeric Unix timestamps to clean human datetime format
        df_ts['datetime_fixed'] = pd.to_datetime(df_ts['reqTimeSec'], unit='s', errors='coerce')
        
        log_start_time = df_ts['datetime_fixed'].min()
        log_end_time = df_ts['datetime_fixed'].max()
        ...
        print("="*65)
        # 🚨 PRINTING TIMELINE DETAILS
        print(f"📅 LOG METRICS WINDOW:")
        print(f"   - File Collection Starts : {log_start_time}")
        print(f"   - File Collection Ends   : {log_end_time}")
        print("-"*65)
