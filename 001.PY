...
def discover_columns_from_folders(input_dirs: list[Path], sample_size_per_folder: int = COLUMN_SAMPLE_SIZE) -> list[str]:
    ...
    for input_dir in input_dirs:
        all_paths = list(input_dir.rglob("*.gz"))
        sample = all_paths[:sample_size_per_folder]
        for fpath in sample:
            try:
                with gzip.open(fpath, "rb") as fh:
                    for raw in fh:
    ...
    def _flush(buf):
        ...
        if writer is None:
            writer = pq.ParquetWriter(
                ...
                compression_level=22 if compress == "zstd" else None
            )
...
def run(cfg: dict):
    ...
    print()
    return {"status": "ok", "output_dir": str(output_dir), "rows": total_rows, "errors": total_errs}
...
def interactive_master_menu() -> dict:
    ...
    while True:
        ...
        if master_dir.exists() and master_dir.is_dir():
            break
        print(f"  ⚠  Folder not found: {master_dir}\n")
    ...
    for d in input_dirs:
        ...
        print(f"  ✓ {d.name}  →  {out.name}  ({status})")
...
def run_master(cfg: dict):
    ...
    for idx, input_dir in enumerate(cfg["input_dirs"], 1):
        output_dir = input_dir.parent / f"{input_dir.name}_parquet"
        print(f"\n{'#'*60}")
        print(f"  Folder {idx}/{len(cfg['input_dirs'])}: {input_dir.name}  →  {output_dir.name}")
        print(f"{'#'*60}")
    ...
    print(f"\n{'='*60}")
    print("  ✓  Master conversion complete!")
    for name, res in results:
        print(f"  {name:<20} → {res.get('status', 'unknown'):<8} rows={res.get('rows', 0):,} errors={res.get('errors', 0):,}")
    print(f"{'='*60}\n")
...
def choose_mode() -> str:
    ...
    banner()
    section("MODE")
    print("  1) Single folder mode  → convert one folder")
    print("  2) Master folder mode  → convert child folders like 01, 02, 03 automatically\n")
